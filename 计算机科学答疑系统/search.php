				...
				<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
					...
					<ul class="nav navbar-nav navbar-right">
						...
						<li>
							<a>欢迎您，
							<?php
								if(isset($_COOKIE['authority'])&&$_COOKIE['authority']=='student') 
		...
		<div class="row">
			...
			<div class="col-md-9">
				...
				<ol class="breadcrumb">
					...
					<?php
					if(isset($_GET['find'])){
					echo "<li class=\"active\">全局查找:".$_GET['find']."</li>";
					}else{
					if (!isset($character)) {
					echo "<li class=\"active\">" . $class . "</li>";
					} else {
					echo "<li><a href=\"search.php?class=" . $class . "\">" . $class . "</a></li>";
					}
					?>
					<?php
					if (isset($character)) {
				...
				<div class="col-md-10">
					<?php
					if(isset($_GET['find'])){
					$sql="SELECT * FROM `queslib` WHERE `question` LIKE '%".trim($_GET['find'])."%'";
					}else{
					if(isset($character))
					$sql = "SELECT *  FROM `queslib` WHERE `character` LIKE '".$character."'";
					else
					$sql = "SELECT *  FROM `queslib` WHERE `class` LIKE '".$class."'";
					}
					$result = mysql_query($sql);
					$row = mysql_fetch_array($result, MYSQL_ASSOC);
					$count=0;
					do{
						if(($row["solved"]==1)&&++$count)
					echo "
					<div class=\"panel panel-default\">
					<a href=\"result.php?id=".$row['Id']."\">
					<div class=\"panel-heading\">
					".htmlentities(trim(substr($row["question"],0,200)),ENT_COMPAT,'UTF-8')."
					</div>
					</a>
					<div class=\"panel-body\">
					".htmlentities(trim(substr($row["describe"],0,200)),ENT_COMPAT,'UTF-8')."
					</div>
					</div>";
					}
					while ($row = mysql_fetch_array($result, MYSQL_ASSOC));
					if($count==0)echo "对不起，找不到相关问题";
					?>
				</div>
			</div>
		</div>
