		...
		<div class="row">
			...
			<div class="col-md-9">
				...
				<div class="col-md-10">
					<?php
					if($_COOKIE['authority']=="teacher"){
						$sql="SELECT * FROM `queslib` WHERE `teacher` LIKE '".trim($ID)."'";
					}else{
						$sql="SELECT * FROM `queslib` WHERE `stuid` LIKE '".trim($ID)."'";
					}
					$result = mysql_query($sql);
					$row = mysql_fetch_array($result, MYSQL_ASSOC);
					$count=0;
					do{
						++$count;
						if($row['solved']==1)$solved="(已解决)";
						else $solved="(未解决)";
					echo "
					<div class=\"panel panel-default\">
					<a href=\"result.php?id=".$row['Id']."\">
					<div class=\"panel-heading\">
					".trim(substr($row["question"],0,200)).$solved."
					</div>
					</a>
					<div class=\"panel-body\">
					".trim(substr($row["describe"],0,200))."
					</div>
					</div>";
					}
					while ($row = mysql_fetch_array($result, MYSQL_ASSOC));
